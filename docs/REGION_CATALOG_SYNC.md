...
동기화 범위는 시·도와 시·군·자치구·군까지만이다. 사용자 검색에서는 이를 `SIDO`, `SIGUNGU`로 해석한다. 읍·면·동 API는 호출하지 않고, 읍·면·동은 검색 단위로 저장하지 않는다.
...
별칭 생성은 공식명 접미사 규칙으로 처리한다. `특별자치도`, `특별자치시`, `특별시`, `광역시`, `시`, `군`, `구`, `도` 순서로 긴 접미사를 먼저 제거하며, 빈 문자열이나 한 글자 별칭은 만들지 않는다.
